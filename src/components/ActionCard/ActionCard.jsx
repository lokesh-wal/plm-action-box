...
export const ActionCard = ({ onClose, currentData, onNext }) => {
  ...
  const renderFormType = () => {
    ...
    if (type === 'select') {
      return <ActionSelect value={selected} onSelect={handleValueSelect} options={cardData.options} />;
    }
    return <></>;
  };
  ...
  return (
    <div className="action-box-card question-card">
      ...
      <div className="centered"><IdeaIcon /></div>
      ...
      <div className="button-wrapper centered">
        <button className={getButtonClass()} onClick={handleNextClick}>{getButtonText()}</button>
      </div>
    </div>
  );
};
